...
cleaned_data = string_or_list(config.get('TRAINING_DATASETS','cleaned_data'))
predicted_actives = string_or_list(config.get('TRAINING_DATASETS','predicted_actives'))
...
df_to_write.to_csv(path_or_buf= cleaned_data,index=False) 
...
def smi_converter(filename):
    ...
    df.to_csv(path_or_buf= predicted_actives,index=False, header=False, sep='\t')
#########SDF GEN
    if sdf_results:
...
def run_pyRMD(config_file_path, output_dir):
    # load the configuration file
    config = configparser.ConfigParser()
    config.read(config_file_path)

    # get the paths of the input files
    chembl_file_path = config['TRAINING_DATASETS']['chembl_file']
    db_to_screen_path = config['MODE']['db_to_screen']

    # load the input files
    chembl_data = pd.read_csv(chembl_file_path)
    db_to_screen = pd.read_csv(db_to_screen_path)

    # run PyRMD
    rmd = RMD(chembl_data)
    predictions = rmd.run(db_to_screen)

    # write the predictions to file
    write_output_to_file(output_dir, predictions)
